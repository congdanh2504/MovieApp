...
class AuthUseCaseImpl @Inject constructor(private val authRepository: AuthRepository): AuthUseCase {
    override suspend fun login(email: String, password: String): Result<Unit> = authRepository.login(email, password)

    override suspend fun register(email: String, username: String, password: String): Result<Unit> = authRepository.register(email, username, password)
